local ASTRO = {
    ...
    Toggles = {},
    Unloaded = false,
    -- Feature tables (mirror gng + mobile additions)
    Features = {
        SilentAim = {
            Enabled = false,
            FOV = 150,
        ...
        AntiAim = { Enabled = false, Pitch = 85, Yaw = 0 },
        AntiKatana = { Enabled = false },
        AutoWalk = { Enabled = false, Method = "Normal" },
        Orbit = { Enabled = false, Radius = 8, Speed = 3, MaxDistance = 400, Height = 3 },
        Stick = { Enabled = false, Smooth = false, Smoothness = 50, Beneath = false, MaxDistance = 400 },
        TeleportKill = { Enabled = false, TargetName = "", Distance = 3, AutoReconnect = true },
        Spoofers = {
            Name = false,
        ...
        AutoWeapon = { Enabled = false, Slots = {} },
        Proximity = { Enabled = false, Distance = 30 },
        AntiAFK = { Enabled = false },
        NameSpoof = { Enabled = false, Name = "ZytheraX", Mode = "Both", Badge = true },
    },
    State = {
...
function ASTRO:MoveMouseRelative(dx, dy)
    ...
    if vim and vim:SendMouseMovementEvent then
        pcall(function()
            ...
            vim:SendMouseMovementEvent(pos.X + dx, pos.Y + dy, Enum.UserInputType.MouseMovement)
        end)
        return true
    end
    return false
...
function ASTRO:FireWeapon()
    ...
    pcall(function()
        if mousebuttonclick then mousebuttonclick(1) return end
    end)
    pcall(function()
        if click then click() return end
    end)
    pcall(function()
...
local function CreateMainWindow()
    ...
    local window = Library:CreateWindow({
        ...
        Footer = "RUNaways Mobile Edition",
...
-- ============================================================
-- SETUP TABS (with group boxes in mind)
-- ============================================================
local function SetupTabs()
    ...
    -- Combat
    tabs.Combat = window:AddTab("Combat", "sword")
    ...
    -- Auto
    tabs.Auto = window:AddTab("Auto Farm", "bot")
    ...
    -- Spoofers
    tabs.Spoofers = window:AddTab("Spoofers", "eye-off")
    ...
    -- Info
    tabs.Info = window:AddTab("Info", "info")
...
local function isTeammate(player)
    if not ASTRO.Features.SilentAim.TeamCheck and not ASTRO.Features.ESP.TeamCheck then return false end
...
-- ============================================================
-- SILENT AIM (with FOV circles, mobile friendly)
-- ============================================================
...
-- ============================================================
-- RAGE BOT (Mobile friendly + FOV circles)
-- ============================================================
        ...
        while not ASTRO.Unloaded do
            if AK.Enabled then
                ...
                elseif not deflecting and state.AntiKatana.WasDeflecting then
                    ...
                    -- Re-enable (UI will handle via toggles)
                end
            end
            task.wait(0.1)
        end
...
function ASTRO:UpdateFovCircles()
    ...
    state.FovCircles.SA.bg.Position = mousePos
    ...
    state.FovCircles.SA.ring.Position = mousePos
    ...
    state.FovCircles.Rage.bg.Position = mousePos
    ...
    state.FovCircles.Rage.ring.Position = mousePos
    ...
    state.FovCircles.Hold.bg.Position = mousePos
...
local function BuildUI()
    ...
    -- ===== SILENT AIM =====
    local SA = Tabs.SilentAim:AddLeftGroupbox("Silent Aim Settings", "target")
    SA:AddToggle("SilentAim", { Text = "Enable", Default = Features.SilentAim.Enabled, Callback = function(v) Features.SilentAim.Enabled = v end })
    SA:AddSlider("SA_FOV", { Text = "FOV Radius", Default = Features.SilentAim.FOV, Min = 50, Max = 500, Rounding = 0, Callback = function(v) Features.SilentAim.FOV = v end })
    SA:AddDropdown("SA_HitPart", { Text = "Hit Part", Values = {"Head","UpperTorso","LowerTorso","HumanoidRootPart"}, Default = 1, Callback = function(v) Features.SilentAim.HitPart = v end })
    SA:AddSlider("SA_Prediction", { Text = "Prediction", Default = Features.SilentAim.Prediction, Min = 0, Max = 0.2, Rounding = 3, Callback = function(v) Features.SilentAim.Prediction = v end })
    SA:AddSlider("SA_MaxDist", { Text = "Max Distance", Default = 2000, Min = 100, Max = 5000, Rounding = 0, Callback = function(v) Features.SilentAim.MaxDistance = v end })
    SA:AddToggle("SA_WallCheck", { Text = "Wall Check", Default = true, Callback = function(v) Features.SilentAim.WallCheck = v end })
    SA:AddToggle("SA_TeamCheck", { Text = "Team Check", Default = false, Callback = function(v) Features.SilentAim.TeamCheck = v end })
    SA:AddDivider()
    SA:AddToggle("SA_FovVisible", { Text = "Show FOV Circle", Default = false, Callback = function(v) Features.SilentAim.FovVisible = v end })
    SA:AddToggle("SA_FovFilled", { Text = "Filled FOV", Default = false, Callback = function(v) Features.SilentAim.FovFilled = v end })
    SA:AddLabel("FOV Color"):AddColorPicker("SA_FovColor", { Default = Color3.fromRGB(255,255,255), Callback = function(c) Features.SilentAim.FovColor = c end })
    SA:AddToggle("SA_FovRainbow", { Text = "Rainbow FOV", Default = false, Callback = function(v) Features.SilentAim.FovRainbow = v end })

    -- ===== RAGE BOT =====
    local RB = Tabs.RageBot:AddLeftGroupbox("Rage Bot Settings", "crosshair")
    RB:AddToggle("RageBot", { Text = "Enable", Default = Features.RageBot.Enabled, Callback = function(v) Features.RageBot.Enabled = v end })
    RB:AddToggle("RageWallbang", { Text = "Wallbang", Default = Features.RageBot.Wallbang, Callback = function(v) Features.RageBot.Wallbang = v end })
    RB:AddSlider("RageFOV", { Text = "FOV Radius", Default = Features.RageBot.FOV, Min = 50, Max = 500, Rounding = 0, Callback = function(v) Features.RageBot.FOV = v end })
    RB:AddSlider("RageSpeed", { Text = "Aim Speed", Default = Features.RageBot.AimSpeed, Min = 0.01, Max = 0.5, Rounding = 2, Callback = function(v) Features.RageBot.AimSpeed = v end })
    RB:AddToggle("RageAutoShot", { Text = "Auto Shoot", Default = true, Callback = function(v) Features.RageBot.AutoShot = v end })
    RB:AddToggle("RageSpamLock", { Text = "Spam Lock (LMB)", Default = false, Callback = function(v) Features.RageBot.SpamLock = v end })
    RB:AddToggle("RageTracer", { Text = "Show Tracer", Default = true, Callback = function(v) Features.RageBot.ShowTracer = v end })
    RB:AddDivider()
    RB:AddToggle("Rage_FovVisible", { Text = "Show FOV Circle", Default = false, Callback = function(v) Features.RageBot.FovVisible = v end })
    RB:AddToggle("Rage_FovFilled", { Text = "Filled FOV", Default = false, Callback = function(v) Features.RageBot.FovFilled = v end })
    RB:AddLabel("FOV Color"):AddColorPicker("Rage_FovColor", { Default = Color3.fromRGB(255,0,0), Callback = function(c) Features.RageBot.FovColor = c end })
    RB:AddToggle("Rage_FovRainbow", { Text = "Rainbow FOV", Default = false, Callback = function(v) Features.RageBot.FovRainbow = v end })

    -- ===== HOLD BOT =====
    local HB = Tabs.HoldBot:AddLeftGroupbox("Hold Bot", "crosshair")
    HB:AddToggle("HoldBot", { Text = "Enable", Default = Features.HoldBot.Enabled, Callback = function(v) Features.HoldBot.Enabled = v end })
    HB:AddSlider("HoldFOV", { Text = "FOV Radius", Default = Features.HoldBot.FOV, Min = 50, Max = 500, Rounding = 0, Callback = function(v) Features.HoldBot.FOV = v end })
    HB:AddSlider("HoldSmooth", { Text = "Smoothing", Default = Features.HoldBot.Smoothing, Min = 1, Max = 20, Rounding = 0, Callback = function(v) Features.HoldBot.Smoothing = v end })
    HB:AddDropdown("HoldHitPart", { Text = "Hit Part", Values = {"Head","UpperTorso","LowerTorso","HumanoidRootPart"}, Default = 1, Callback = function(v) Features.HoldBot.HitPart = v end })
    HB:AddSlider("HoldMaxDist", { Text = "Max Distance", Default = Features.HoldBot.MaxDistance, Min = 100, Max = 3000, Rounding = 0, Callback = function(v) Features.HoldBot.MaxDistance = v end })
    HB:AddToggle("HoldWall", { Text = "Wall Check", Default = true, Callback = function(v) Features.HoldBot.TargetBehindWalls = not v end })
    HB:AddDivider()
    HB:AddToggle("Hold_FovVisible", { Text = "Show FOV Circle", Default = false, Callback = function(v) Features.HoldBot.FovVisible = v end })
    HB:AddToggle("Hold_FovFilled", { Text = "Filled FOV", Default = false, Callback = function(v) Features.HoldBot.FovFilled = v end })
    HB:AddLabel("FOV Color"):AddColorPicker("Hold_FovColor", { Default = Color3.fromRGB(0,255,255), Callback = function(c) Features.HoldBot.FovColor = c end })
    HB:AddToggle("Hold_FovRainbow", { Text = "Rainbow FOV", Default = false, Callback = function(v) Features.HoldBot.FovRainbow = v end })

    -- ===== TRIGGER BOT =====
    local TB = Tabs.TriggerBot:AddLeftGroupbox("Trigger Bot", "bot")
    TB:AddToggle("TriggerBot", { Text = "Enable", Default = Features.TriggerBot.Enabled, Callback = function(v) Features.TriggerBot.Enabled = v end })
    TB:AddSlider("TriggerDelay", { Text = "Delay (s)", Default = Features.TriggerBot.Delay, Min = 0.01, Max = 0.3, Rounding = 2, Callback = function(v) Features.TriggerBot.Delay = v end })
    TB:AddToggle("TriggerWall", { Text = "Wall Check", Default = true, Callback = function(v) Features.TriggerBot.WallCheck = v end })
    TB:AddToggle("TriggerKeybind", { Text = "Hold RMB to Fire", Default = false, Callback = function(v) Features.TriggerBot.Keybind = v end })

    -- ===== ANTI-AIM =====
    local AA = Tabs.AntiAim:AddLeftGroupbox("Anti-Aim", "shield")
    AA:AddToggle("AntiAim", { Text = "Enable", Default = Features.AntiAim.Enabled, Callback = function(v) Features.AntiAim.Enabled = v end })
    AA:AddSlider("AAPitch", { Text = "Pitch (Lookdown)", Default = Features.AntiAim.Pitch, Min = 0, Max = 90, Rounding = 0, Suffix = "°", Callback = function(v) Features.AntiAim.Pitch = v end })
    AA:AddSlider("AAYaw", { Text = "Yaw Offset", Default = Features.AntiAim.Yaw, Min = -180, Max = 180, Rounding = 0, Suffix = "°", Callback = function(v) Features.AntiAim.Yaw = v end })

    -- ===== ANTI-KATANA =====
    local AK = Tabs.AntiKatana:AddLeftGroupbox("Anti-Katana", "katana")
    AK:AddToggle("AntiKatana", { Text = "Enable", Default = Features.AntiKatana.Enabled, Callback = function(v) Features.AntiKatana.Enabled = v end })
    AK:AddLabel("Detects katana/sword in ViewModels and pauses aim features.", true)

    -- ===== ESP PLAYERS =====
    local ESPG = Tabs.ESPPlayers:AddLeftGroupbox("ESP Players", "users")
    ESPG:AddToggle("EspBoxes", { Text = "Boxes", Default = Features.ESP.Boxes, Callback = function(v) Features.ESP.Boxes = v end })
    ESPG:AddToggle("EspFilled", { Text = "Filled Boxes", Default = Features.ESP.FilledBoxes, Callback = function(v) Features.ESP.FilledBoxes = v end })
    ESPG:AddToggle("EspTracers", { Text = "Tracers", Default = Features.ESP.Tracers, Callback = function(v) Features.ESP.Tracers = v end })
    ESPG:AddToggle("EspHealth", { Text = "Health Bars", Default = Features.ESP.Health, Callback = function(v) Features.ESP.Health = v end })
    ESPG:AddToggle("EspNames", { Text = "Names", Default = Features.ESP.Names, Callback = function(v) Features.ESP.Names = v end })
    ESPG:AddToggle("EspDistance", { Text = "Distance", Default = Features.ESP.Distance, Callback = function(v) Features.ESP.Distance = v end })
    ESPG:AddToggle("EspChams", { Text = "Chams", Default = Features.ESP.Chams, Callback = function(v) Features.ESP.Chams = v end })
    ESPG:AddToggle("EspGlow", { Text = "Glow Chams", Default = Features.ESP.GlowChams, Callback = function(v) Features.ESP.GlowChams = v end })
    ESPG:AddToggle("EspSkeleton", { Text = "Skeleton", Default = Features.ESP.Skeleton, Callback = function(v) Features.ESP.Skeleton = v end })
    ESPG:AddToggle("EspArrow", { Text = "Arrow ESP", Default = Features.ESP.Arrow, Callback = function(v) Features.ESP.Arrow = v end })
    ESPG:AddToggle("EspTeam", { Text = "Team Check (Skip teammates)", Default = false, Callback = function(v) Features.ESP.TeamCheck = v end })
    ESPG:AddSlider("EspMaxDist", { Text = "Max Distance", Default = 400, Min = 100, Max = 2000, Rounding = 0, Callback = function(v) Features.ESP.MaxDistance = v end })
    ESPG:AddSlider("EspBoxThick", { Text = "Box Thickness", Default = 1.5, Min = 1, Max = 5, Rounding = 1, Callback = function(v) Features.ESP.BoxThickness = v end })
    ESPG:AddSlider("EspHeadScale", { Text = "Head Scale", Default = 1, Min = 1, Max = 3, Rounding = 0.1, Callback = function(v) Features.ESP.HeadScale = v end })

    -- ===== ESP WORLD =====
    local TRIP = Tabs.ESPWorld:AddLeftGroupbox("Tripmine ESP", "eye")
    TRIP:AddToggle("TripmineESP", { Text = "Enable", Default = false, Callback = function(v) Features.ESP.Tripmine = v end })
    local EW = Tabs.ESPWorld:AddRightGroupbox("Enemy Weapons Panel", "package")
    EW:AddToggle("EnemyWeapons", { Text = "Show Panel", Default = false, Callback = function(v) Features.ESP.EnemyWeapons = v end })

    -- ===== GUN MODS =====
    local GMG = Tabs.GunMain:AddLeftGroupbox("Gun Mods", "sliders")
    GMG:AddToggle("GunMaster", { Text = "Master Enable", Default = Features.GunMods.Master, Callback = function(v) Features.GunMods.Master = v end })
    GMG:AddToggle("GunNoRecoil", { Text = "No Recoil", Default = Features.GunMods.NoRecoil, Callback = function(v) Features.GunMods.NoRecoil = v end })
    GMG:AddToggle("GunNoSpread", { Text = "No Spread", Default = Features.GunMods.NoSpread, Callback = function(v) Features.GunMods.NoSpread = v end })
    GMG:AddToggle("GunRapid", { Text = "Rapid Fire", Default = Features.GunMods.RapidFire, Callback = function(v) Features.GunMods.RapidFire = v end })
    GMG:AddToggle("GunOneShot", { Text = "One Shot Kill", Default = Features.GunMods.OneShot, Callback = function(v) Features.GunMods.OneShot = v end })
    GMG:AddToggle("GunInfAmmo", { Text = "Infinite Ammo", Default = Features.GunMods.InfiniteAmmo, Callback = function(v) Features.GunMods.InfiniteAmmo = v end })
    GMG:AddToggle("GunInstantReload", { Text = "Instant Reload", Default = Features.GunMods.InstantReload, Callback = function(v) Features.GunMods.InstantReload = v end })
    GMG:AddToggle("GunInstantEquip", { Text = "Instant Equip", Default = Features.GunMods.InstantEquip, Callback = function(v) Features.GunMods.InstantEquip = v end })
    GMG:AddToggle("GunNoDrop", { Text = "No Bullet Drop", Default = Features.GunMods.NoBulletDrop, Callback = function(v) Features.GunMods.NoBulletDrop = v end })
    GMG:AddToggle("GunMaxPierce", { Text = "Max Pierce", Default = Features.GunMods.MaxPierce, Callback = function(v) Features.GunMods.MaxPierce = v end })
    GMG:AddToggle("GunNoCooldown", { Text = "No Cooldowns", Default = Features.GunMods.NoCooldowns, Callback = function(v) Features.GunMods.NoCooldowns = v end })

    -- ===== MOVEMENT =====
    local MV = Tabs.MoveMain:AddLeftGroupbox("Movement", "footprints")
    MV:AddToggle("MoveSpeed", { Text = "Custom Walk Speed", Default = Features.Movement.WalkSpeedEnabled, Callback = function(v) Features.Movement.WalkSpeedEnabled = v end })
    MV:AddSlider("MoveSpeedVal", { Text = "Walk Speed", Default = Features.Movement.WalkSpeed, Min = 16, Max = 200, Rounding = 0, Callback = function(v) Features.Movement.WalkSpeed = v end })
    MV:AddToggle("MoveJump", { Text = "Custom Jump Power", Default = Features.Movement.JumpPowerEnabled, Callback = function(v) Features.Movement.JumpPowerEnabled = v end })
    MV:AddSlider("MoveJumpVal", { Text = "Jump Power", Default = Features.Movement.JumpPower, Min = 1, Max = 300, Rounding = 0, Callback = function(v) Features.Movement.JumpPower = v end })
    MV:AddToggle("MoveNoclip", { Text = "Noclip", Default = Features.Movement.Noclip, Callback = function(v) Features.Movement.Noclip = v end })
    MV:AddToggle("MoveInfJump", { Text = "Infinite Jump", Default = Features.Movement.InfiniteJump, Callback = function(v) Features.Movement.InfiniteJump = v end })
    MV:AddToggle("MoveAirWalk", { Text = "Air Walk", Default = Features.Movement.AirWalk, Callback = function(v) Features.Movement.AirWalk = v end })
    MV:AddToggle("MoveSlide", { Text = "Slide Boost", Default = Features.Movement.SlideBoost, Callback = function(v) Features.Movement.SlideBoost = v end })
    MV:AddSlider("MoveSlidePower", { Text = "Slide Power", Default = Features.Movement.SlideBoostPower, Min = 1, Max = 15, Rounding = 1, Callback = function(v) Features.Movement.SlideBoostPower = v end })
    MV:AddToggle("MoveBhop", { Text = "Auto Bhop", Default = Features.Movement.AutoBhop, Callback = function(v) Features.Movement.AutoBhop = v end })
    MV:AddSlider("MoveGravity", { Text = "Gravity", Default = Features.Movement.Gravity, Min = 10, Max = 400, Rounding = 0, Callback = function(v) Features.Movement.Gravity = v end })

    -- Fly
    local FLY = Tabs.MoveFly:AddLeftGroupbox("Fly", "plane")
    FLY:AddToggle("FlyEnable", { Text = "Enable Fly", Default = Features.Movement.Fly, Callback = function(v) Features.Movement.Fly = v; if v then ASTRO.startFly(Features.Movement.FlySpeed) else ASTRO.stopFly() end end })
    FLY:AddSlider("FlySpeed", { Text = "Fly Speed", Default = Features.Movement.FlySpeed, Min = 10, Max = 500, Rounding = 0, Callback = function(v) Features.Movement.FlySpeed = v; if Features.Movement.Fly then ASTRO.stopFly(); task.wait(0.05); ASTRO.startFly(v) end end })

    -- Air Movement
    local AIR = Tabs.MoveAir:AddLeftGroupbox("Air Movement", "wind")
    AIR:AddToggle("AirStrafe", { Text = "Air Strafe", Default = Features.Movement.AirStrafe, Callback = function(v) Features.Movement.AirStrafe = v end })
    AIR:AddSlider("AirStrafeStr", { Text = "Strength", Default = Features.Movement.AirStrafeStrength, Min = 5, Max = 100, Rounding = 0, Callback = function(v) Features.Movement.AirStrafeStrength = v end })
    AIR:AddToggle("AutoJump", { Text = "Auto Jump", Default = Features.Movement.AutoJump, Callback = function(v) Features.Movement.AutoJump = v end })
    AIR:AddToggle("CircleStrafe", { Text = "Circle Strafe", Default = Features.Movement.CircleStrafe, Callback = function(v) Features.Movement.CircleStrafe = v end })
    AIR:AddToggle("QuickStop", { Text = "Quick Stop", Default = Features.Movement.QuickStop, Callback = function(v) Features.Movement.QuickStop = v end })

    -- ===== VISUALS =====
    local VISG = Tabs.VisMain:AddLeftGroupbox("Visuals", "paintbrush")
    VISG:AddToggle("VisCrosshair", { Text = "Crosshair", Default = Features.Visuals.Crosshair, Callback = function(v) Features.Visuals.Crosshair = v end })
    VISG:AddToggle("VisSky", { Text = "Sky Override", Default = Features.Visuals.SkyColor, Callback = function(v) Features.Visuals.SkyColor = v end })
    VISG:AddLabel("Sky Color"):AddColorPicker("VisSkyColor", { Default = Color3.fromRGB(80,80,100), Callback = function(c) Features.Visuals.SkyColorVal = c end })
    VISG:AddSlider("VisSkyBright", { Text = "Brightness", Default = Features.Visuals.SkyBrightness, Min = 0, Max = 3, Rounding = 2, Callback = function(v) Features.Visuals.SkyBrightness = v end })
    VISG:AddSlider("VisSkyTime", { Text = "Time of Day", Default = Features.Visuals.SkyClockTime, Min = 0, Max = 24, Rounding = 1, Callback = function(v) Features.Visuals.SkyClockTime = v end })
    VISG:AddToggle("VisFog", { Text = "Fog", Default = Features.Visuals.Fog, Callback = function(v) Features.Visuals.Fog = v end })
    VISG:AddSlider("VisFogDist", { Text = "Fog Distance", Default = Features.Visuals.FogDistance, Min = 100, Max = 10000, Rounding = 0, Callback = function(v) Features.Visuals.FogDistance = v end })
    VISG:AddToggle("VisThird", { Text = "Third Person", Default = Features.Visuals.Thirdperson, Callback = function(v) Features.Visuals.Thirdperson = v end })
    VISG:AddSlider("VisThirdDist", { Text = "Distance", Default = Features.Visuals.ThirdpersonDistance, Min = 5, Max = 30, Rounding = 0, Callback = function(v) Features.Visuals.ThirdpersonDistance = v end })
    VISG:AddToggle("VisFOV", { Text = "FOV Override", Default = Features.Visuals.FOVOverride, Callback = function(v) Features.Visuals.FOVOverride = v end })
    VISG:AddSlider("VisFOVVal", { Text = "FOV Value", Default = Features.Visuals.FOVValue, Min = 70, Max = 170, Rounding = 0, Callback = function(v) Features.Visuals.FOVValue = v end })
    VISG:AddToggle("VisLockInd", { Text = "Lock Indicator", Default = Features.Visuals.LockIndicator, Callback = function(v) Features.Visuals.LockIndicator = v end })
    VISG:AddToggle("VisSmoke", { Text = "Hide Smoke", Default = Features.Visuals.HideSmoke, Callback = function(v) Features.Visuals.HideSmoke = v end })
    VISG:AddToggle("VisFlash", { Text = "Hide Flash", Default = Features.Visuals.HideFlash, Callback = function(v) Features.Visuals.HideFlash = v end })
    VISG:AddToggle("VisBloom", { Text = "Bloom", Default = Features.Visuals.Bloom, Callback = function(v) Features.Visuals.Bloom = v end })
    VISG:AddSlider("VisBloomInt", { Text = "Bloom Intensity", Default = Features.Visuals.BloomIntensity, Min = 1, Max = 100, Rounding = 0, Callback = function(v) Features.Visuals.BloomIntensity = v end })
    VISG:AddToggle("VisTracer", { Text = "Bullet Tracer", Default = Features.Visuals.BulletTracer, Callback = function(v) Features.Visuals.BulletTracer = v end })
    VISG:AddDropdown("VisTracerColor", { Text = "Tracer Color", Values = {"Red","Green","Pink","Toothpaste","White"}, Default = 4, Callback = function(v) Features.Visuals.BulletTracerColor = v end })
    VISG:AddSlider("VisTracerLifetime", { Text = "Trail Lifetime", Default = Features.Visuals.BulletTracerLifetime, Min = 1, Max = 30, Rounding = 0, Callback = function(v) Features.Visuals.BulletTracerLifetime = v end })
    VISG:AddSlider("VisTracerSpeed", { Text = "Bullet Speed", Default = Features.Visuals.BulletTracerSpeed, Min = 100, Max = 2000, Rounding = 0, Callback = function(v) Features.Visuals.BulletTracerSpeed = v end })
    VISG:AddToggle("VisWeaponLatex", { Text = "Weapon Latex", Default = Features.Visuals.WeaponLatex, Callback = function(v) Features.Visuals.WeaponLatex = v end })
    VISG:AddToggle("VisArmLatex", { Text = "Arm Latex", Default = Features.Visuals.ArmLatex, Callback = function(v) Features.Visuals.ArmLatex = v end })

    -- Crosshair SubTab
    local CHG = Tabs.VisCrosshair:AddLeftGroupbox("Crosshair", "target")
    CHG:AddDropdown("CrosshairColor", { Text = "Color", Values = {"Red","Blue","Purple","Yellow","Pink","Orange","Cyan"}, Default = 3, Callback = function(v) Features.Visuals.CrosshairColor = v end })

    -- Weather SubTab
    local WTH = Tabs.WorldWeather:AddLeftGroupbox("Weather", "cloud-rain")
    WTH:AddDropdown("WeatherType", { Text = "Weather", Values = {"None","Rain","Snow"}, Default = 1, Callback = function(v) Features.Visuals.Weather = v end })
    WTH:AddToggle("NightMode", { Text = "Night Mode", Default = Features.Visuals.NightMode, Callback = function(v) Features.Visuals.NightMode = v end })

    -- ===== AUTO =====
    local AUTO = Tabs.AutoMain:AddLeftGroupbox("Auto Farm", "play")
    AUTO:AddToggle("AutoLoot", { Text = "Auto Loot (Aura)", Default = false, Callback = function(v) ASTRO.Toggles.LootAura = v end })
    AUTO:AddToggle("AntiAFK", { Text = "Anti AFK", Default = Features.AntiAFK.Enabled, Callback = function(v) Features.AntiAFK.Enabled = v end })

    -- Auto Walk
    local AWG = Tabs.AutoWalk:AddLeftGroupbox("Auto Walk", "footprints")
    AWG:AddToggle("AutoWalk", { Text = "Enable", Default = Features.AutoWalk.Enabled, Callback = function(v) Features.AutoWalk.Enabled = v end })
    AWG:AddDropdown("AutoWalkMethod", { Text = "Method", Values = {"Normal","Strafing","Flanking"}, Default = 1, Callback = function(v) Features.AutoWalk.Method = v end })

    -- Orbit
    local ORBG = Tabs.AutoOrbit:AddLeftGroupbox("Orbit", "circle")
    ORBG:AddToggle("Orbit", { Text = "Enable", Default = Features.Orbit.Enabled, Callback = function(v) Features.Orbit.Enabled = v end })
    ORBG:AddSlider("OrbitRadius", { Text = "Radius", Default = Features.Orbit.Radius, Min = 3, Max = 30, Rounding = 1, Callback = function(v) Features.Orbit.Radius = v end })
    ORBG:AddSlider("OrbitSpeed", { Text = "Speed", Default = Features.Orbit.Speed, Min = 0.5, Max = 15, Rounding = 1, Callback = function(v) Features.Orbit.Speed = v end })
    ORBG:AddSlider("OrbitHeight", { Text = "Height", Default = Features.Orbit.Height, Min = 0, Max = 15, Rounding = 1, Callback = function(v) Features.Orbit.Height = v end })
    ORBG:AddSlider("OrbitMaxDist", { Text = "Max Distance", Default = Features.Orbit.MaxDistance, Min = 100, Max = 2000, Rounding = 0, Callback = function(v) Features.Orbit.MaxDistance = v end })

    -- Stick
    local STG = Tabs.AutoStick:AddLeftGroupbox("Stick to Target", "user-plus")
    STG:AddToggle("Stick", { Text = "Enable", Default = Features.Stick.Enabled, Callback = function(v) Features.Stick.Enabled = v end })
    STG:AddToggle("StickSmooth", { Text = "Smooth", Default = Features.Stick.Smooth, Callback = function(v) Features.Stick.Smooth = v end })
    STG:AddSlider("StickSmoothness", { Text = "Smoothness", Default = Features.Stick.Smoothness, Min = 0, Max = 100, Rounding = 0, Callback = function(v) Features.Stick.Smoothness = v end })
    STG:AddToggle("StickBeneath", { Text = "Beneath Player", Default = Features.Stick.Beneath, Callback = function(v) Features.Stick.Beneath = v end })
    STG:AddSlider("StickMaxDist", { Text = "Max Distance", Default = Features.Stick.MaxDistance, Min = 100, Max = 2000, Rounding = 0, Callback = function(v) Features.Stick.MaxDistance = v end })

    -- Auto Weapon Pick
    local AWPG = Tabs.AutoWeapon:AddLeftGroupbox("Auto Weapon Pick", "package")
    AWPG:AddToggle("AutoWeapon", { Text = "Enable", Default = Features.AutoWeapon.Enabled, Callback = function(v) Features.AutoWeapon.Enabled = v end })
    for i = 1, 4 do
        AWPG:AddInput("WeaponSlot"..i, { Text = "Slot "..i, Default = "", Finished = true, Callback = function(v) Features.AutoWeapon.Slots[i] = v end })
    end

    -- ===== TELEPORT =====
    local TP = Tabs.TeleportMain:AddLeftGroupbox("Teleport Kill", "map-pin")
    TP:AddInput("TpTarget", { Text = "Target Name", Default = "", Finished = true, Callback = function(v) Features.TeleportKill.TargetName = v end })
    TP:AddSlider("TpDist", { Text = "Distance", Default = 3, Min = 1, Max = 20, Rounding = 0, Callback = function(v) Features.TeleportKill.Distance = v end })
    TP:AddToggle("TpAutoReconnect", { Text = "Auto Reconnect", Default = true, Callback = function(v) Features.TeleportKill.AutoReconnect = v end })
    TP:AddButton("Start Teleport", function() ASTRO.TeleportKill.start() end)
    TP:AddButton("Stop Teleport", function() ASTRO.TeleportKill.stop() end)

    -- ===== REWARDS =====
    local RW = Tabs.RewardsMain:AddLeftGroupbox("Rewards", "gift")
    RW:AddButton("Claim All Rewards", function() ASTRO:ClaimAllRewards() end)
    RW:AddButton("Redeem All Codes", function() ASTRO:RedeemAllCodes() end)

    -- ===== SPOOFERS =====
    local NAM = Tabs.SpoofName:AddLeftGroupbox("Name Spoofer", "user")
    NAM:AddToggle("NameSpoof", { Text = "Enable", Default = Features.NameSpoof.Enabled, Callback = function(v) Features.NameSpoof.Enabled = v end })
    NAM:AddInput("NameSpoofName", { Text = "Spoofed Name", Default = "ZytheraX", Finished = true, Callback = function(v) Features.NameSpoof.Name = v end })
    NAM:AddDropdown("NameSpoofMode", { Text = "Mode", Values = {"Both","Name","DisplayName"}, Default = 1, Callback = function(v) Features.NameSpoof.Mode = v end })
    NAM:AddToggle("NameSpoofBadge", { Text = "Add Verified Badge ✓", Default = true, Callback = function(v) Features.NameSpoof.Badge = v end })

    local DEV = Tabs.SpoofDevice:AddLeftGroupbox("Device Spoofer", "smartphone")
    DEV:AddToggle("DeviceSpoof", { Text = "Enable", Default = Features.Spoofers.Device, Callback = function(v) Features.Spoofers.Device = v end })
    DEV:AddDropdown("DeviceTarget", { Text = "Target", Values = {"Controller","PC","Mobile","VR"}, Default = 1, Callback = function(v) Features.Spoofers.DeviceTarget = v end })

    local LVL = Tabs.SpoofLevel:AddLeftGroupbox("Level / Winstreak", "chart")
    LVL:AddToggle("LevelSpoof", { Text = "Spoof Level", Default = Features.Spoofers.Level, Callback = function(v) Features.Spoofers.Level = v end })
    LVL:AddSlider("LevelVal", { Text = "Level", Default = 996, Min = 1, Max = 9999, Rounding = 0, Callback = function(v) Features.Spoofers.LevelVal = v end })
    LVL:AddToggle("WinstreakSpoof", { Text = "Spoof Winstreak", Default = Features.Spoofers.Winstreak, Callback = function(v) Features.Spoofers.Winstreak = v end })
    LVL:AddSlider("WinstreakVal", { Text = "Winstreak", Default = 56, Min = 0, Max = 9999, Rounding = 0, Callback = function(v) Features.Spoofers.WinstreakVal = v end })

    -- ===== PROXIMITY =====
    local PROX = Tabs.MiscProx:AddLeftGroupbox("Proximity Alert", "map")
    PROX:AddToggle("Proximity", { Text = "Enable", Default = Features.Proximity.Enabled, Callback = function(v) Features.Proximity.Enabled = v end })
    PROX:AddSlider("ProxDist", { Text = "Alert Distance", Default = 30, Min = 10, Max = 200, Rounding = 0, Callback = function(v) Features.Proximity.Distance = v end })

    -- ===== SERVER =====
    local SRV = Tabs.PlayerServer:AddLeftGroupbox("Server", "server")
    SRV:AddButton("Server Hop", function() ASTRO:ServerHop() end)
    SRV:AddButton("Rejoin", function() ASTRO:Rejoin() end)

    -- ===== INFO =====
    local INFO = Tabs.Info:AddLeftGroupbox("About", "info")
    INFO:AddLabel(ASTRO.Name .. " - " .. ASTRO.SubName, true)
    INFO:AddLabel("Version: " .. ASTRO.Version, true)
    INFO:AddLabel("Full feature port from ZytheraX / RUNaways", true)
    INFO:AddLabel("Optimized for Mobile", true)
    INFO:AddLabel("Powered by Obsidian Library", true)
...
function ASTRO:Init()
    ...
    print("[ASTRO] Loading full feature set (Mobile optimized)...")
    ...
    print("[ASTRO] All features loaded! Press LeftAlt to toggle UI.")
    self:Notify("ASTRO - Full Feature Set Loaded! (Mobile Ready)", 5)
    return self
end
