...
local function SetupTabs()
    ...
    tabs.Info = window:AddTab("ℹ️ Info")
...
local function getCrystalRarity(child)
    ...
    return ASTRO.State.rarityList[tier] or "Unknown"
end
...
local function buildRuneCache()
    ...
    task.spawn(function()
        ...
        state.scanRuneLock = false
        if ASTRO._updateRuneLabel then ASTRO._updateRuneLabel() end
    end)
end
...
local function buildAllCache()
    buildCrystalCache()
    buildRuneCache()
    if ASTRO._updateCrystalLabel then ASTRO._updateCrystalLabel() end
end
...
local function setupKickListeners()
    ...
    -- Listener 2: CoreGui prompt scan
    task.spawn(function()
...
-- ============================================================
-- BUILD UI (Semua fitur di UI Library)
-- ============================================================
function ASTRO:BuildUI()
    ...
    -- ===== DUPE TAB =====
    local dupeGroup = Tabs.Dupe:AddLeftGroupbox("Drop Items", "package")

    -- Crystal dropdown
    dupeGroup:AddDropdown("CrystalsToDrop", {
        Text = "Crystals to Drop",
    ...
    -- Crystal cache label (sebagai informasi)
    dupeGroup:AddLabel("Crystal Cache: click Scan to update", true)
    ...
    dupeGroup:AddLabel("Rune Cache: click Scan to update", true)
    ...
    dupeGroup:AddButton("🗑 Manual Drop Selected", function()
        task.spawn(function()
            executeDropItems()
    ...
    rejoinGroup:AddInput("PrivateServerLink", {
        ...
        Placeholder = "Paste full URL VIP",
        Finished = true,
        Callback = function(v)
    ...
    infoGroup:AddLabel("Based on D-Hub Dupe Standalone", true)
...
function ASTRO:Init()
    ...
    -- Inisialisasi scan inventory
    scanInventory()

    -- Build UI
    self:BuildUI()

    -- Setup kick listeners
    setupKickListeners()
