...
-- update check
local update = false -- Set ke true untuk enable update check, atau biarkan false
if update then
...
if not loadSuccess or not Library or not Loader then
	warn("[Rick] Failed to load UI assets. Check asset IDs.")
	return
end
...
function syde:Destroy()
	...
	if Library then
		pcall(function() Library.Enabled = false end)
		task.wait(0.1)
		pcall(function() Library:Destroy() end)
	end

	for _, connData in ipairs(self.Connections or {}) do
		if connData.Connection and connData.Connection.Connected then
			pcall(function() connData.Connection:Disconnect() end)
		end
	end

	for uid, _ in pairs(binds or {}) do
		pcall(function() RunService:UnbindFromRenderStep(uid) end)
	end

	for frame, _ in pairs(binds or {}) do
		binds[frame] = nil
	end
	...
	-- Cleanup wiggle texts
	pcall(function()
		for _, v in ipairs(coregui:GetDescendants()) do
			if v:IsA("Frame") and v.Name == "WiggleContainer" then
...
function syde:AddConnection(Type, Callback, AutoCleanup)
	...
	local ConnectionData = { Connection = Connection, AutoCleanup = AutoCleanup ~= false }
...
function Bento:Destroy()
	if not self then return end
	if self.ActiveTweens then
		for frame, tw in pairs(self.ActiveTweens) do
...
do
	...
	function syde:Load(Config)
		...
		if not Loader then
			warn("[RICK] Loader asset not loaded")
			return
		end
		local LOADER = Loader
		LOADER.Enabled = true
		...
		-- PreLoad
		Config.Name = Config.Name or 'Rick'
		...
		local function load()
			...
			if Config.ConfigurationSaving and Config.ConfigurationSaving.Enabled then
				local folderName = Config.ConfigurationSaving.FolderName or "RickConfig"
				...
				if isfolder and not isfolder(folderName) then
					...
					if not success then
						warn("[RICK] Failed to create folder:", err)
					end
				end
				...
				if not success then
					warn("[RICK] File write/read blocked or failed:", err)
				end
			end
			...
			local UI_TAG = "rickUILoader"
			local MARKER_NAME = "RICKUIDetector"
			local INTERNAL_UUID = ("RICK-" .. tostring(game.JobId):gsub("-", "") .. tostring(tick())):gsub("%.", "")
			...
			-- Load the Library
			local successLibrary, loadedLib = pcall(function()
				return Library -- Replace with actual GetObjects if needed
			end)

			if not successLibrary or not loadedLib then
				warn("[RICK] Failed to load Library.")
				return
			end
			local Library = loadedLib
			...
			task.spawn(function()
				while Library and Library.Parent do
					...
					if Library.Parent ~= coregui then
						warn("Rick 〡 UI moved. Restoring...")
						pcall(function()
							Library.Parent = coregui
						end)
					end
			...
			if Config.AutoJoinDiscord and Config.AutoJoinDiscord.Enabled then
				...
				local rootFolder = Config.ConfigurationSaving and Config.ConfigurationSaving.FolderName or "RickConfig"
				...
				warn("[RICK] Discord AutoJoin enabled. Preparing to send invite:", inviteCode)
				...
				if isfolder and not isfolder(discordFolder) then
					...
					if folderSuccess then
						print("[RICK] Created DiscordInvites folder at:", discordFolder)
					else
						warn("[RICK] Failed to create DiscordInvites folder:", folderErr)
					end
				end

				local shouldPrompt = true
				if isfile and discordConfig.RememberJoins and isfile(inviteFilePath) then
					print("[RICK] Discord invite '" .. inviteCode .. "' already joined. Skipping prompt.")
					shouldPrompt = false
				end
				...
				if shouldPrompt then
					if request then
						...
						if reqSuccess then
							print("[RICK] Discord invite sent successfully to:", inviteCode)
						else
							warn("[RICK] Failed to send Discord invite:", reqErr)
						end
					else
						warn("[RICK] Request function not available — cannot send Discord invite.")
					end
					...
					if discordConfig.RememberJoins then
						...
						if writeSuccess then
							print("[RICK] Logged Discord invite join for:", inviteCode)
						else
							warn("[RICK] Failed to write join log for invite:", writeErr)
						end
					end
				end
			end
...
local function PerformSave()
	...
	if not success then
		warn("Rick 〡 Failed to save config:", err)
	end
end
	...
	task.spawn(function()
		...
		local function CloseNotif()
			...
			if Notification and Notification.Parent then
				...
				task.wait(0.4)
				if Notification:FindFirstChild("close") then
					pcall(function() Notification.close:Destroy() end)
				end
...
window.top.functions.close.interact.MouseButton1Click:Connect(function()
	syde:Modal({
		...
		ConfirmCallBack = function()
			mh = false
			if rs then pcall(function() rs:Disconnect() end) end
			if ss then pcall(function() ss:Disconnect() end) end
			if syde.FPSConnection then pcall(function() syde.FPSConnection:Disconnect() end) end
			task.wait(1)
			syde:Destroy()
		end,
	})
...
function syde:Init(library)
	...
	if mh then
		syde.FPSConnection = RunService.RenderStepped:Connect(function()
			-- FPS
			frames += 1
			local now = tick()
			...
			Minihome.info.time.Text = hour .. os.date(":%M")
		end)
		rs = syde.FPSConnection
	...
	syde:MakeResizable(window.resize, window, Vector2.new(454, 228), function(newSize)
		task.defer(function()
						...
						for _, otherPicker in pairs(Page:GetChildren()) do
							if otherPicker:IsA("Frame") and otherPicker:FindFirstChild("isLinkable") and otherPicker.isLinkable.Value then
								if isMouseOver(otherPicker) and otherPicker ~= draggedColorPicker then
									...
									-- Let HueSat.Changed on otherPicker trigger its own updatestuff
									foundTarget = true
									TweenService:Create(
	...
	function tbdata:InitTab(tab)
		...
		local function SwitchToTab(tabName)
			...
			if not (selectedTab and targetPage) then
				warn("[Rick] SwitchToTab failed:", tabName)
				return
			end
		...
		function syde:LoadSaveConfig()
			...
			if syde.ConfigEnabled then
				...
				if success then
					...
					warn("[RICK] Configurations Error " .. tostring(result))
				end
			end
		end
...
-- ============================================================
-- RickUI Public API
-- Wrapper layer agar bisa dipanggil dengan:
--   local RickUI = loadstring(game:HttpGet("..."))()
--   local Window = RickUI:CreateWindow({ Title = "Test" })
--   local Tab    = Window:CreateTab({ Title = "Main" })
--   Tab:CreateToggle({ Title = "ESP", Default = false, Callback = function(v) end })
-- ============================================================

local RickUI = {}

-- ──────────────────────────────────────────────
-- Notify (global shortcut)
-- ──────────────────────────────────────────────
function RickUI:Notify(data)
	syde:Notify({
...
-- CreateWindow
-- ──────────────────────────────────────────────
function RickUI:CreateWindow(config)
	config = config or {}

	-- Map config keys ke format yang dipakai syde:Init()
	local libraryConfig = {
		Title    = config.Title    or "Rick UI";
		SubText  = config.SubText  or config.Subtitle or "";
		Accent   = config.Accent;
		Logo     = config.Logo;
		Home     = config.Home     or { Enabled = false };
		ConfigurationSaving = config.ConfigurationSaving;
		AutoJoinDiscord     = config.AutoJoinDiscord;
		Socials             = config.Socials;
	}

	-- Kalau ada Accent override di config, apply ke theme
	if config.Accent then
		syde:UpdateTheme({ Accent = config.Accent, HitBox = config.Accent })
	end

	-- Panggil Loader kalau ada; kalau tidak langsung Init
	if config.Loader then
		syde:Loader(config.Loader)
	else
		syde:Init(libraryConfig)
	end

	-- tbdata ada di dalam scope syde:Init, kita perlu akses lewat
	-- syde:CreateTab yang kita bungkus di sini
	local Window = {}
	...
	function Window:CreateTab(tabConfig)
		...
		-- Panggil tbdata:InitTab yang ada di dalam syde:Init
		-- syde:Init mengembalikan tbdata, tapi kita tidak menyimpannya di atas.
		-- Solusi: kita expose syde._tbdata lewat syde:Init patch di bawah.
		local tabHandle = syde._tbdata and syde._tbdata:InitTab({
		...
		if not tabHandle then
			warn("[RickUI] CreateTab: tbdata not ready. Pastikan CreateWindow sudah dipanggil.")
			return {}
		end
		...
		function Tab:CreateSection(title)
			tabHandle:Section(title or "Section")
		end

		-- ── Label ──
		function Tab:CreateLabel(text, alignment)
			tabHandle:Label(text or "", alignment)
		end
		...
		function Tab:CreateButton(btnConfig)
			...
			tabHandle:Button({
				...
				Description = btnConfig.Desc        or btnConfig.Description or "";
				Type        = btnConfig.Type        or "Default";
				HoldTime    = btnConfig.HoldTime    or 3;
				CallBack    = btnConfig.Callback    or btnConfig.CallBack;
			})
		end
		...
		function Tab:CreateToggle(togConfig)
			...
			tabHandle:Toggle({
				Title       = togConfig.Title       or "Toggle";
				Description = togConfig.Desc        or togConfig.Description or "";
				Value       = togConfig.Default     ~= nil and togConfig.Default or false;
				Config      = togConfig.Config      or false;
				Flag        = togConfig.Flag;
				CallBack    = togConfig.Callback    or togConfig.CallBack;
			})
		end
		...
		function Tab:CreateSlider(sliderConfig)
			...
			-- Slider internal pakai format { Sliders = { {...} } }
			tabHandle:Slider({
				Title       = sliderConfig.Title or "Slider";
				Description = sliderConfig.Desc  or sliderConfig.Description or "";
				Sliders = {
					{
						Title        = sliderConfig.Title    or "Value";
						Range        = { sliderConfig.Min or 0, sliderConfig.Max or 100 };
						Increment    = sliderConfig.Increment or 1;
						StarterValue = sliderConfig.Default   or sliderConfig.StarterValue or 50;
						Flag         = sliderConfig.Flag;
						CallBack     = sliderConfig.Callback  or sliderConfig.CallBack;
					}
				};
			})
		end
		...
		function Tab:CreateDropdown(ddConfig)
			...
			tabHandle:Dropdown({
				Title       = ddConfig.Title    or "Dropdown";
				Description = ddConfig.Desc     or ddConfig.Description or "";
				Options     = ddConfig.Options  or {};
				Default     = ddConfig.Default;
				Multi       = ddConfig.Multi    or false;
				PlaceHolder = ddConfig.PlaceHolder or "Select...";
				Flag        = ddConfig.Flag;
				CallBack    = ddConfig.Callback or ddConfig.CallBack;
			})
		end
		...
		function Tab:CreateColorPicker(cpConfig)
			...
			tabHandle:ColorPicker({
				Title    = cpConfig.Title    or "Color";
				Color    = cpConfig.Default  or cpConfig.Color or Color3.fromRGB(255, 255, 255);
				Flag     = cpConfig.Flag;
				CallBack = cpConfig.Callback or cpConfig.CallBack;
			})
		end
		...
		function Tab:CreateKeybind(kbConfig)
			...
			tabHandle:Keybind({
				Title       = kbConfig.Title    or "Keybind";
				Key         = kbConfig.Default  or kbConfig.Key;
				Description = kbConfig.Desc     or kbConfig.Description or "";
				CallBack    = kbConfig.Callback or kbConfig.CallBack;
			})
		end

		-- ── TextInput ──
		function Tab:CreateTextInput(tiConfig)
			tiConfig = tiConfig or {}
			tabHandle:TextInput({
				Title       = tiConfig.Title       or "Input";
				PlaceHolder = tiConfig.PlaceHolder or "Type here...";
				NumberOnly  = tiConfig.NumberOnly  or false;
				ClearOnLost = tiConfig.ClearOnLost ~= nil and tiConfig.ClearOnLost or true;
				CallBack    = tiConfig.Callback    or tiConfig.CallBack;
			})
		end
		...
		function Tab:CreateParagraph(pConfig)
			...
			tabHandle:Paragraph({
				...
				Content = pConfig.Content or "";
			})
		end

		-- ── 3D View ──
		function Tab:CreateEnhancedView(viewConfig)
			viewConfig = viewConfig or {}
			tabHandle:EnchancedView({
				Title      = viewConfig.Title      or "3D View";
				Object     = viewConfig.Object;
				UserRotate = viewConfig.UserRotate or false;
				AutoRotate = viewConfig.AutoRotate ~= false;
			})
		end
	...
	-- ────────────────────────────────────────────
	-- Window:Notify (shortcut di level window)
	-- ────────────────────────────────────────────
	...
	-- ────────────────────────────────────────────
	-- Window:SetTheme (opsional override warna)
	-- ────────────────────────────────────────────
	function Window:SetTheme(themeTable)
		syde:UpdateTheme(themeTable)
	end

	-- ────────────────────────────────────────────
	-- Window:Toggle (buka/tutup UI)
	-- ────────────────────────────────────────────
	function Window:Toggle()
		if typeof(ToggleUI) == "function" then
			ToggleUI()
		end
	end
...
-- ──────────────────────────────────────────────
-- Patch syde:Init supaya expose tbdata keluar
-- Ini diperlukan agar Window:CreateTab bisa
-- mengakses tbdata yang ada di closure Init
-- ──────────────────────────────────────────────
local _originalInit = syde.Init
function syde:Init(library)
	local result = _originalInit(self, library)
	-- tbdata dikembalikan oleh Init sebagai result
	if result and type(result) == "table" and result.InitTab then
		syde._tbdata = result
	end
	return result
end
