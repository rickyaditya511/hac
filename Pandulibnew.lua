...
local DEFAULT_LOGO = "rbxassetid://94380161420025"
...
local function buildTagFrame(player)
    ...
    root.Name              = "panduTag_" .. player.UserId
...
local function applyOutline(player)
    ...
    local existing = char:FindFirstChild("panduOutline")
    if existing then existing:Destroy() end

    local hl = Instance.new("Highlight")
    hl.Name             = "panduOutline"
...
local function addTag(player)
    ...
    local conn = RunService.RenderStepped:Connect(function(dt)
        ...
        local outline = char:FindFirstChild("panduOutline")
...
local function tagRegister()
    ...
    if sok and type(data) == "table" and data.kick == true then
        pcall(function() lp:Kick("[pandu] Disconnected by admin") end)
    end
end
...
function Library:SetTheme(theme)
    ...
    if type(theme) == "string" then
        themeName = theme
        theme = THEMES[theme]
        if not theme then warn(("[pandu UI] unknown theme %q"):format(themeName)); return false end
    elseif type(theme) ~= "table" then
        warn("[pandu UI] SetTheme expects a built-in theme name or theme table"); return false
    end
    for key in pairs(C) do
        local value = theme[key]
        if value ~= nil and typeof(value) ~= "Color3" then
            warn(("[pandu UI] theme key %s must be a Color3"):format(key)); return false
        end
    end
    for key in pairs(C) do
        local value = theme[key]
...
function Library:SaveConfig(name)
    if not hasFileApi() then
        warn("[pandu UI] SaveConfig requires an executor file API (writefile)")
        return false
    end
    ensureConfigFolder()
    ...
    if not ok then warn("[pandu UI] SaveConfig failed to encode config"); return false end
    local wrote = pcall(writefile, configPath(name), encoded)
    if not wrote then warn("[pandu UI] SaveConfig failed to write file"); return false end
    return true
end

function Library:LoadConfig(name)
    if not hasFileApi() then
        warn("[pandu UI] LoadConfig requires an executor file API (readfile)")
        return false
    end
    ...
    if not decoded then warn("[pandu UI] LoadConfig failed to decode config"); return false end
...
function Library:Notify(opts)
    ...
    warn("[pandu UI] create a window before calling Library:Notify")
    return nil
end
...
local function buildMusicPlayer(cfg)
    ...
    local MUSIC_FOLDER   = tostring(opts.MusicFolder or "panduMusic")
    ...
    musicSound.Name    = "panduMusicPlayer"
    ...
    local BUILTIN_TRACKS = {
        { name = "pandu Anthem", id = "rbxassetid://75485931767123", startTime = 3, endTime = 115 },
...
function Library:CreateWindow(opts)
    ...
    local guiName        = opts.GuiName or "panduUI"
    ...
    local loadingText         = tostring(opts.LoadingText or opts.Name or "pandu")
    local loadingSub          = tostring(opts.LoadingSubtitle or "HUB")
    local loadingFooter       = tostring(opts.LoadingFooter or "pandu HUB")
    ...
    make("TextLabel",{Text=opts.Name or "pandu UI",Font=Enum.Font.GothamBold,TextSize=13,TextColor3=C.White,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,BackgroundTransparency=1,Position=UDim2.fromOffset(64,16),Size=UDim2.new(1,-72,0,17),Parent=brand})
    make("TextLabel",{Text=opts.BrandSubtitle or ("pandu v"..Library.Version),Font=Enum.Font.GothamMedium,TextSize=9,TextColor3=C.TextDim,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,BackgroundTransparency=1,Position=UDim2.fromOffset(64,35),Size=UDim2.new(1,-72,0,13),Parent=brand})
    ...
    circle(statusDot)
    make("TextLabel",{Text=opts.StatusText or "pandu is ready",Font=Enum.Font.GothamMedium,TextSize=10,TextColor3=C.TextDim,TextXAlignment=Enum.TextXAlignment.Left,BackgroundTransparency=1,Position=UDim2.new(0,28,1,-27),Size=UDim2.new(1,-40,0,16),Parent=sidebar})
    ...
    if isMobile then
        local fab = make("TextButton", {
            Name = "panduMobileToggle", Text = "", AutoButtonColor = false,
