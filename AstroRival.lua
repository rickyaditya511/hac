local ASTRO = {
    ...
    Toggles = {},
    Unloaded = false,
    -- Feature tables (mirror gng)
    Features = {
        SilentAim = { Enabled = false, FOV = 150, HitPart = "Head", Prediction = 0.10, MaxDistance = 2000, WallCheck = true, TeamCheck = false, ProjectilePrediction = false },
        RageBot = { Enabled = false, Wallbang = false, FOV = 250, AimSpeed = 0.18, ShowTracer = true, TracerStart = "Cursor", TracerColor = Color3.fromRGB(255,50,50), TracerThickness = 1, SpamLock = false, AutoShot = true },
        ESP = { Boxes = false, FilledBoxes = false, Tracers = false, Health = false, Names = false, Distance = false, Chams = false, GlowChams = false, Skeleton = false, Arrow = false, EnemyWeapons = false, TeamCheck = false, MaxDistance = 400, BoxThickness = 1.5, LineThickness = 1, BoxSizeMultiplier = 1200, FilledTransparency = 0.4, ChamsBrightness = 5, GlowBrightness = 3, HeadScale = 1 },
        GunMods = { Master = false, NoRecoil = false, NoSpread = false, RapidFire = false, FireRateMultiplier = 1.0, OneShot = false, InfiniteAmmo = false, InstantReload = false, InstantEquip = false, NoBulletDrop = false, MaxPierce = false, NoCooldowns = false, ZeroSpreadIL = false, ZeroRecoilIL = false },
        Movement = { WalkSpeedEnabled = false, WalkSpeed = 50, JumpPowerEnabled = false, JumpPower = 50, Noclip = false, InfiniteJump = false, AirWalk = false, SlideBoost = false, SlideBoostPower = 4, Fly = false, FlySpeed = 80, AutoBhop = false, AirStrafe = false, AirStrafeStrength = 20, AutoJump = false, CircleStrafe = false, QuickStop = false, Gravity = 196 },
        Visuals = { Crosshair = false, CrosshairColor = "Purple", SkyColor = false, SkyColorVal = Color3.fromRGB(80,80,100), SkyBrightness = 1.5, SkyClockTime = 12, HideSmoke = false, HideFlash = false, LockIndicator = false, BulletTracer = false, BulletTracerColor = "Toothpaste", BulletTracerLifetime = 10, BulletTracerSpeed = 600, WeaponLatex = false, ArmLatex = false, Bloom = false, BloomIntensity = 20, Thirdperson = false, ThirdpersonDistance = 12, ThirdpersonKey = "Always on", FOVOverride = false, FOVValue = 120, WorldColor = false, WorldColorName = "white", Fog = false, FogColorName = "white", FogDistance = 1000, Weather = "None", NightMode = false },
        ...
        Spoofers = { Name = false, SpoofedName = "ZytheraX", EnemyName = "Johnny", Device = false, DeviceTarget = "Controller", Level = false, LevelVal = 996, Winstreak = false, WinstreakVal = 56 },
...
-- ============================================================
-- SILENT AIM (full __namecall hook)
-- ============================================================
...
-- ============================================================
-- RAGE BOT (Wallbang + Tracer + Spam Lock)
-- ============================================================
    ...
    local rageConn = RunService.RenderStepped:Connect(function()
        ...
        if target then
            ...
            if onScreen then
                ...
                if mousemoverel then pcall(function() mousemoverel(delta.X, delta.Y) end) end
            end
            local now = tick()
        ...
        if RB.SpamLock and uis:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
            ...
            if target3 then
                ...
                if on then
                    ...
                    if mousemoverel then pcall(function() mousemoverel(moveX, moveY) end) end
                end
            end
        end
    end)
...
-- ============================================================
-- HOLD BOT (Smooth Aimbot)
-- ============================================================
local function SetupHoldBot()
    local HB = ASTRO.Features.HoldBot or { Enabled = false, FOV = 250, Smoothing = 3, Prediction = false, MaxDistance = 2000, HitPart = "Head", TargetBehindWalls = false }
    local lp = ASTRO.Services.LocalPlayer
    local uis = ASTRO.Services.UserInputService
    local camera = ASTRO.Services.Camera
    local RunService = ASTRO.Services.RunService
...
local function SetupTriggerBot()
    local TB = ASTRO.Features.TriggerBot or { Enabled = false, Delay = 0.05, WallCheck = true, Keybind = false }
    local lp = ASTRO.Services.LocalPlayer
    local uis = ASTRO.Services.UserInputService
    local camera = ASTRO.Services.Camera
    local RunService = ASTRO.Services.RunService
...
-- ============================================================
-- ESP (Full: Box, Filled, Tracer, Health, Names, Distance, Chams, Glow, Skeleton, Arrow, EnemyWeapons)
-- ============================================================
...
local function SetupTeleportKill()
    ...
    local RunService = ASTRO.Services.RunService
    local state = ASTRO.State
    state.TeleportKill = { Conn = nil, Target = nil }
...
-- ============================================================
-- BUILD UI (All toggles and controls)
-- ============================================================
local function BuildUI()
    ...
    -- ===== SILENT AIM =====
    local SA = Tabs.SilentAim:AddLeftGroupbox("Silent Aim", "target")
    ...
    SA:AddSlider("SA_FOV", { Text = "FOV", Default = Features.SilentAim.FOV, Min = 50, Max = 500, Rounding = 0, Callback = function(v) Features.SilentAim.FOV = v end })
    ...
    -- ===== RAGE BOT =====
    local RB = Tabs.RageBot:AddLeftGroupbox("Rage Bot", "crosshair")
    ...
    RB:AddSlider("RageFOV", { Text = "FOV", Default = Features.RageBot.FOV, Min = 50, Max = 500, Rounding = 0, Callback = function(v) Features.RageBot.FOV = v end })
    ...
    HB:AddToggle("HoldBot", { Text = "Enable", Default = false, Callback = function(v) Features.HoldBot.Enabled = v end })
    HB:AddSlider("HoldFOV", { Text = "FOV", Default = 250, Min = 50, Max = 500, Rounding = 0, Callback = function(v) Features.HoldBot.FOV = v end })
    HB:AddSlider("HoldSmooth", { Text = "Smoothing", Default = 3, Min = 1, Max = 20, Rounding = 0, Callback = function(v) Features.HoldBot.Smoothing = v end })
    ...
    TB:AddToggle("TriggerBot", { Text = "Enable", Default = false, Callback = function(v) Features.TriggerBot.Enabled = v end })
    TB:AddSlider("TriggerDelay", { Text = "Delay", Default = 0.05, Min = 0.01, Max = 0.3, Rounding = 2, Callback = function(v) Features.TriggerBot.Delay = v end })
    ...
    AA:AddToggle("AntiAim", { Text = "Enable", Default = false, Callback = function(v) Features.AntiAim.Enabled = v end })
    AA:AddSlider("AAPitch", { Text = "Pitch", Default = 85, Min = 0, Max = 90, Rounding = 0, Callback = function(v) Features.AntiAim.Pitch = v end })
    AA:AddSlider("AAYaw", { Text = "Yaw", Default = 0, Min = -180, Max = 180, Rounding = 0, Callback = function(v) Features.AntiAim.Yaw = v end })

    -- ===== ANTI-KATANA =====
    local AK = Tabs.AntiKatana:AddLeftGroupbox("Anti-Katana", "katana")
    AK:AddToggle("AntiKatana", { Text = "Enable", Default = false, Callback = function(v) Features.AntiKatana.Enabled = v end })

    -- ===== ESP =====
    local ESPG = Tabs.ESPPlayers:AddLeftGroupbox("ESP", "users")
    ESPG:AddToggle("EspBoxes", { Text = "Boxes", Default = false, Callback = function(v) Features.ESP.Boxes = v end })
    ESPG:AddToggle("EspFilled", { Text = "Filled Boxes", Default = false, Callback = function(v) Features.ESP.FilledBoxes = v end })
    ESPG:AddToggle("EspTracers", { Text = "Tracers", Default = false, Callback = function(v) Features.ESP.Tracers = v end })
    ESPG:AddToggle("EspHealth", { Text = "Health Bars", Default = false, Callback = function(v) Features.ESP.Health = v end })
    ESPG:AddToggle("EspNames", { Text = "Names", Default = false, Callback = function(v) Features.ESP.Names = v end })
    ESPG:AddToggle("EspDistance", { Text = "Distance", Default = false, Callback = function(v) Features.ESP.Distance = v end })
    ESPG:AddToggle("EspChams", { Text = "Chams", Default = false, Callback = function(v) Features.ESP.Chams = v end })
    ESPG:AddToggle("EspGlow", { Text = "Glow Chams", Default = false, Callback = function(v) Features.ESP.GlowChams = v end })
    ESPG:AddToggle("EspSkeleton", { Text = "Skeleton", Default = false, Callback = function(v) Features.ESP.Skeleton = v end })
    ESPG:AddToggle("EspArrow", { Text = "Arrow ESP", Default = false, Callback = function(v) Features.ESP.Arrow = v end })
    ESPG:AddToggle("EspTeam", { Text = "Team Check", Default = false, Callback = function(v) Features.ESP.TeamCheck = v end })
    ...
    ESPG:AddSlider("EspHeadScale", { Text = "Head Scale", Default = 1, Min = 1, Max = 3, Rounding = 1, Callback = function(v) Features.ESP.HeadScale = v end })
    ...
    -- Enemy Weapons
    local EW = Tabs.ESPWorld:AddRightGroupbox("Enemy Weapons", "package")
    ...
    GMG:AddToggle("GunMaster", { Text = "Master Enable", Default = false, Callback = function(v) Features.GunMods.Master = v end })
    GMG:AddToggle("GunNoRecoil", { Text = "No Recoil", Default = false, Callback = function(v) Features.GunMods.NoRecoil = v end })
    GMG:AddToggle("GunNoSpread", { Text = "No Spread", Default = false, Callback = function(v) Features.GunMods.NoSpread = v end })
    GMG:AddToggle("GunRapid", { Text = "Rapid Fire", Default = false, Callback = function(v) Features.GunMods.RapidFire = v end })
    GMG:AddToggle("GunOneShot", { Text = "One Shot", Default = false, Callback = function(v) Features.GunMods.OneShot = v end })
    GMG:AddToggle("GunInfAmmo", { Text = "Infinite Ammo", Default = false, Callback = function(v) Features.GunMods.InfiniteAmmo = v end })
    GMG:AddToggle("GunInstantReload", { Text = "Instant Reload", Default = false, Callback = function(v) Features.GunMods.InstantReload = v end })
    GMG:AddToggle("GunInstantEquip", { Text = "Instant Equip", Default = false, Callback = function(v) Features.GunMods.InstantEquip = v end })
    GMG:AddToggle("GunNoDrop", { Text = "No Bullet Drop", Default = false, Callback = function(v) Features.GunMods.NoBulletDrop = v end })
    GMG:AddToggle("GunMaxPierce", { Text = "Max Pierce", Default = false, Callback = function(v) Features.GunMods.MaxPierce = v end })
    GMG:AddToggle("GunNoCooldown", { Text = "No Cooldowns", Default = false, Callback = function(v) Features.GunMods.NoCooldowns = v end })

    -- ===== MOVEMENT =====
    local MV = Tabs.MoveMain:AddLeftGroupbox("Movement", "footprints")
    MV:AddToggle("MoveSpeed", { Text = "Custom Walk Speed", Default = false, Callback = function(v) Features.Movement.WalkSpeedEnabled = v end })
    MV:AddSlider("MoveSpeedVal", { Text = "Walk Speed", Default = 50, Min = 16, Max = 200, Rounding = 0, Callback = function(v) Features.Movement.WalkSpeed = v end })
    MV:AddToggle("MoveJump", { Text = "Custom Jump Power", Default = false, Callback = function(v) Features.Movement.JumpPowerEnabled = v end })
    MV:AddSlider("MoveJumpVal", { Text = "Jump Power", Default = 50, Min = 1, Max = 300, Rounding = 0, Callback = function(v) Features.Movement.JumpPower = v end })
    MV:AddToggle("MoveNoclip", { Text = "Noclip", Default = false, Callback = function(v) Features.Movement.Noclip = v end })
    MV:AddToggle("MoveInfJump", { Text = "Infinite Jump", Default = false, Callback = function(v) Features.Movement.InfiniteJump = v end })
    MV:AddToggle("MoveAirWalk", { Text = "Air Walk", Default = false, Callback = function(v) Features.Movement.AirWalk = v end })
    MV:AddToggle("MoveSlide", { Text = "Slide Boost", Default = false, Callback = function(v) Features.Movement.SlideBoost = v end })
    MV:AddSlider("MoveSlidePower", { Text = "Slide Power", Default = 4, Min = 1, Max = 15, Rounding = 1, Callback = function(v) Features.Movement.SlideBoostPower = v end })
    MV:AddToggle("MoveBhop", { Text = "Auto Bhop", Default = false, Callback = function(v) Features.Movement.AutoBhop = v end })
    MV:AddSlider("MoveGravity", { Text = "Gravity", Default = 196, Min = 10, Max = 400, Rounding = 0, Callback = function(v) Features.Movement.Gravity = v end })

    -- Fly
    local FLY = Tabs.MoveFly:AddLeftGroupbox("Fly", "plane")
    FLY:AddToggle("FlyEnable", { Text = "Enable Fly", Default = false, Callback = function(v) Features.Movement.Fly = v; if v then ASTRO.startFly(Features.Movement.FlySpeed) else ASTRO.stopFly() end end })
    FLY:AddSlider("FlySpeed", { Text = "Fly Speed", Default = 80, Min = 10, Max = 500, Rounding = 0, Callback = function(v) Features.Movement.FlySpeed = v; if Features.Movement.Fly then ASTRO.stopFly(); task.wait(0.05); ASTRO.startFly(v) end end })

    -- Air Movement
    local AIR = Tabs.MoveAir:AddLeftGroupbox("Air Movement", "wind")
    AIR:AddToggle("AirStrafe", { Text = "Air Strafe", Default = false, Callback = function(v) Features.Movement.AirStrafe = v end })
    AIR:AddSlider("AirStrafeStr", { Text = "Strength", Default = 20, Min = 5, Max = 100, Rounding = 0, Callback = function(v) Features.Movement.AirStrafeStrength = v end })
    AIR:AddToggle("AutoJump", { Text = "Auto Jump", Default = false, Callback = function(v) Features.Movement.AutoJump = v end })
    AIR:AddToggle("CircleStrafe", { Text = "Circle Strafe", Default = false, Callback = function(v) Features.Movement.CircleStrafe = v end })
    AIR:AddToggle("QuickStop", { Text = "Quick Stop", Default = false, Callback = function(v) Features.Movement.QuickStop = v end })

    -- ===== VISUALS =====
    local VISG = Tabs.VisMain:AddLeftGroupbox("Visuals", "paintbrush")
    VISG:AddToggle("VisCrosshair", { Text = "Crosshair", Default = false, Callback = function(v) Features.Visuals.Crosshair = v end })
    VISG:AddDropdown("VisCrosshairColor", { Text = "Color", Values = {"Red","Blue","Purple","Yellow","Pink","Orange","Cyan"}, Default = 3, Callback = function(v) Features.Visuals.CrosshairColor = v end })
    VISG:AddToggle("VisSky", { Text = "Sky Override", Default = false, Callback = function(v) Features.Visuals.SkyColor = v end })
    ...
    VISG:AddSlider("VisSkyBright", { Text = "Brightness", Default = 1.5, Min = 0, Max = 3, Rounding = 2, Callback = function(v) Features.Visuals.SkyBrightness = v end })
    VISG:AddSlider("VisSkyTime", { Text = "Time of Day", Default = 12, Min = 0, Max = 24, Rounding = 1, Callback = function(v) Features.Visuals.SkyClockTime = v end })
    VISG:AddToggle("VisFog", { Text = "Fog", Default = false, Callback = function(v) Features.Visuals.Fog = v end })
    VISG:AddSlider("VisFogDist", { Text = "Fog Distance", Default = 1000, Min = 100, Max = 10000, Rounding = 0, Callback = function(v) Features.Visuals.FogDistance = v end })
    VISG:AddToggle("VisThird", { Text = "Third Person", Default = false, Callback = function(v) Features.Visuals.Thirdperson = v end })
    VISG:AddSlider("VisThirdDist", { Text = "Third Dist", Default = 12, Min = 5, Max = 30, Rounding = 0, Callback = function(v) Features.Visuals.ThirdpersonDistance = v end })
    VISG:AddToggle("VisFOV", { Text = "FOV Override", Default = false, Callback = function(v) Features.Visuals.FOVOverride = v end })
    VISG:AddSlider("VisFOVVal", { Text = "FOV Value", Default = 120, Min = 70, Max = 170, Rounding = 0, Callback = function(v) Features.Visuals.FOVValue = v end })
    VISG:AddToggle("VisLockInd", { Text = "Lock Indicator", Default = false, Callback = function(v) Features.Visuals.LockIndicator = v end })
    VISG:AddToggle("VisSmoke", { Text = "Hide Smoke", Default = false, Callback = function(v) Features.Visuals.HideSmoke = v end })
    VISG:AddToggle("VisFlash", { Text = "Hide Flash", Default = false, Callback = function(v) Features.Visuals.HideFlash = v end })
    VISG:AddToggle("VisBloom", { Text = "Bloom", Default = false, Callback = function(v) Features.Visuals.Bloom = v end })
    VISG:AddSlider("VisBloomInt", { Text = "Bloom Intensity", Default = 20, Min = 1, Max = 100, Rounding = 0, Callback = function(v) Features.Visuals.BloomIntensity = v end })
    VISG:AddToggle("VisTracer", { Text = "Bullet Tracer", Default = false, Callback = function(v) Features.Visuals.BulletTracer = v end })
    ...
    VISG:AddSlider("VisTracerLifetime", { Text = "Trail Lifetime", Default = 10, Min = 1, Max = 30, Rounding = 0, Callback = function(v) Features.Visuals.BulletTracerLifetime = v end })
    VISG:AddSlider("VisTracerSpeed", { Text = "Bullet Speed", Default = 600, Min = 100, Max = 2000, Rounding = 0, Callback = function(v) Features.Visuals.BulletTracerSpeed = v end })
    VISG:AddToggle("VisWeaponLatex", { Text = "Weapon Latex", Default = false, Callback = function(v) Features.Visuals.WeaponLatex = v end })
    VISG:AddToggle("VisArmLatex", { Text = "Arm Latex", Default = false, Callback = function(v) Features.Visuals.ArmLatex = v end })
    ...
    WTH:AddToggle("NightMode", { Text = "Night Mode", Default = false, Callback = function(v) Features.Visuals.NightMode = v end })
    ...
    AUTO:AddToggle("AntiAFK", { Text = "Anti AFK", Default = false, Callback = function(v) Features.AntiAFK.Enabled = v end })

    -- Auto Walk
    local AWG = Tabs.AutoWalk:AddLeftGroupbox("Auto Walk", "footprints")
    AWG:AddToggle("AutoWalk", { Text = "Enable", Default = false, Callback = function(v) Features.AutoWalk.Enabled = v end })
    ...
    ORBG:AddToggle("Orbit", { Text = "Enable", Default = false, Callback = function(v) Features.Orbit.Enabled = v end })
    ORBG:AddSlider("OrbitRadius", { Text = "Radius", Default = 8, Min = 3, Max = 30, Rounding = 1, Callback = function(v) Features.Orbit.Radius = v end })
    ORBG:AddSlider("OrbitSpeed", { Text = "Speed", Default = 3, Min = 0.5, Max = 15, Rounding = 1, Callback = function(v) Features.Orbit.Speed = v end })
    ORBG:AddSlider("OrbitHeight", { Text = "Height", Default = 3, Min = 0, Max = 15, Rounding = 1, Callback = function(v) Features.Orbit.Height = v end })
    ORBG:AddSlider("OrbitMaxDist", { Text = "Max Distance", Default = 400, Min = 100, Max = 2000, Rounding = 0, Callback = function(v) Features.Orbit.MaxDistance = v end })

    -- Stick
    local STG = Tabs.AutoStick:AddLeftGroupbox("Stick to Target", "user-plus")
    STG:AddToggle("Stick", { Text = "Enable", Default = false, Callback = function(v) Features.Stick.Enabled = v end })
    STG:AddToggle("StickSmooth", { Text = "Smooth", Default = false, Callback = function(v) Features.Stick.Smooth = v end })
    STG:AddSlider("StickSmoothness", { Text = "Smoothness", Default = 50, Min = 0, Max = 100, Rounding = 0, Callback = function(v) Features.Stick.Smoothness = v end })
    STG:AddToggle("StickBeneath", { Text = "Beneath Player", Default = false, Callback = function(v) Features.Stick.Beneath = v end })
    STG:AddSlider("StickMaxDist", { Text = "Max Distance", Default = 400, Min = 100, Max = 2000, Rounding = 0, Callback = function(v) Features.Stick.MaxDistance = v end })

    -- Auto Weapon Pick
    local AWPG = Tabs.AutoWeapon:AddLeftGroupbox("Auto Weapon Pick", "package")
    AWPG:AddToggle("AutoWeapon", { Text = "Enable", Default = false, Callback = function(v) Features.AutoWeapon.Enabled = v end })
    for i = 1, 4 do
    ...
    -- ===== SPOOFERS =====
    local NAM = Tabs.SpoofName:AddLeftGroupbox("Name Spoof", "user")
    NAM:AddToggle("NameSpoof", { Text = "Enable", Default = false, Callback = function(v) Features.NameSpoof.Enabled = v end })
    ...
    DEV:AddToggle("DeviceSpoof", { Text = "Enable", Default = false, Callback = function(v) Features.Spoofers.Device = v end })
    ...
    LVL:AddToggle("LevelSpoof", { Text = "Spoof Level", Default = false, Callback = function(v) Features.Spoofers.Level = v end })
    ...
    LVL:AddToggle("WinstreakSpoof", { Text = "Spoof Winstreak", Default = false, Callback = function(v) Features.Spoofers.Winstreak = v end })
    ...
    PROX:AddToggle("Proximity", { Text = "Enable", Default = false, Callback = function(v) Features.Proximity.Enabled = v end })
    PROX:AddSlider("ProxDist", { Text = "Distance", Default = 30, Min = 10, Max = 200, Rounding = 0, Callback = function(v) Features.Proximity.Distance = v end })
...
function ASTRO:Notify(text, duration)
    if self.Library then
        self.Library:Notify({ Title = self.Name, Description = text, Time = duration or 4 })
    end
end

function ASTRO:Init()
    if self.Loaded then return self end
    print("[ASTRO] Loading full feature set...")
    ...
    self:Notify("ASTRO - Full Feature Set Loaded!", 5)
    return self
end
